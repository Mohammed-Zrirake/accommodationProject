...
// --- Reusable Styles ---
const textInputStyle = "w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 transition-colors text-gray-700";
const labelStyle = "block text-sm font-medium text-gray-700 mb-1";
const sectionTitleStyle = "text-lg font-semibold text-gray-800 mb-4 pb-2 border-b border-gray-200";
const checkboxStyle = "h-4 w-4 rounded border-gray-300 text-purple-600 focus:ring-purple-500";
const buttonPrimary = "bg-purple-600 hover:bg-purple-700 text-white px-6 py-2 rounded-lg font-medium shadow-sm transition-colors focus:outline-none focus:ring-2 focus:ring-purple-500 focus:ring-offset-2";
const buttonSecondary = "bg-white hover:bg-gray-50 text-gray-700 px-6 py-2 rounded-lg font-medium border border-gray-300 shadow-sm transition-colors focus:outline-none focus:ring-2 focus:ring-purple-500 focus:ring-offset-2";
...
const DormAmenitiesSelector = ({ selectedAmenities, onToggleAmenity }) => {
    ...
    if (isLoading) return <div className="animate-pulse mt-4 space-y-2">
    ...
    if (error) return <p className="mt-4 text-sm font-medium text-red-500 bg-red-50 p-2 rounded">{error}</p>;

    return (
        <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-200">
            <h5 className={labelStyle}>Dorm Amenities</h5>
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-x-4 gap-y-3 mt-2">
                {allAmenities.map(amenity => (
                    <div key={amenity.amenityId} className="flex items-center">
                        ...
                        />
                        <label htmlFor={`dorm-amenity-${amenity.amenityId}`} className="ml-2 text-sm text-gray-700">
                            {amenity.name}
                        </label>
                    </div>
                ))}
...
const DormImageUploader = ({ photos, onImageChange }) => (
    <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-200">
        <h5 className={labelStyle}>Dorm Photos</h5>
        <div className='grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mt-2'>
            {Object.keys(photos).map((key) => (
                <label 
                    htmlFor={`dormImage${key}`} 
                    key={key} 
                    className="relative group cursor-pointer border-2 border-dashed border-gray-300 rounded-lg h-28 flex items-center justify-center bg-gray-50 hover:border-purple-400 transition-colors overflow-hidden"
                >
                    {photos[key] ? (
                        <>
                            <img 
...
const AddDormForm = ({ onAddDorm, onCancel }) => {
    ...
    const handleSubmit = (e) => {
        ...
        onAddDorm({ ...formData, photos: photoFiles });
    }

    return (
        <form onSubmit={handleSubmit} className="p-6 rounded-lg bg-white shadow-md border border-gray-200 space-y-6">
            <h4 className={sectionTitleStyle}>Add New Dorm</h4>
            
            {/* Basic Info */}
            <div className="space-y-4">
                <div>
                    <label htmlFor="name" className={labelStyle}>Dorm Name</label>
                    <input 
                        type="text" 
                        name="name" 
                        id="name"
                        placeholder="8-Bed Female Dormitory" 
                        value={formData.name} 
                ...
                <div className="grid md:grid-cols-3 gap-4">
                    ...
                    <div>
                        <label htmlFor="basePricePerNight" className={labelStyle}>Price Per Night ($)</label>
                        <input 
                            type="number" 
                    ...
                    <div className="flex items-end">
                        <div className="w-full">
                            <label htmlFor="isAvailable" className={labelStyle}>Availability</label>
            ...
            <div className="space-y-4">
                <div>
                    ...
                    <textarea 
                        name="description" 
                        id="description"
                        placeholder="Describe the dorm and its facilities..." 
                ...
                <div>
                    ...
                    <textarea 
                        name="rules" 
                        id="rules"
                        placeholder="Quiet hours, Age restrictions, No smoking, etc..." 
